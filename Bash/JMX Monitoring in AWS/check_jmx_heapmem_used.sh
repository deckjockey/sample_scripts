...
OUTPUT=$($1/java -cp $RDIR/jmxquery.jar org.nagios.JMXQuery -U service:jmx:rmi:///jndi/rmi://localhost:8089/jmxrmi -O java.lang:type=Memory -A HeapMemoryUsage -K "used" -u B | cut -d '=' -f2)
...
OUTPUT=$(aws cloudwatch put-metric-data --metric-name HeapSpaceUsage --namespace JMX --unit Bytes --value $HEAPSIZE --dimensions InstanceId=$INSTANCEID --region $REGIONNAME)
